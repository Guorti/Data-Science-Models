...
Para obtener un ajuste con regularizacion Lasso, se indica el argumento $\alpha =0$, ademas se sugiere que el parametro $\lambda$ se inicie en 100, o de lo contrario la funcion lo asignara en un rango automatico que tiene por defecto.
...
paste("MAPE:", MAPE, "%")
```
Modelos por debajo del 10% de error se buscan

# Ejercicio:
* Seleccionar una variable diferente y ejecutar los modelos Lasso y Ridge, para (TO, TD, Ocupados)
* Comparar los coeficientes en cada uno de los modelos
* Comparar los MAPE en cada uno de los 6 modelos (2 por cada variable)
* Cual es la programacion del modelo Elastic Net ( Modelo que une Ridge y Lasso) aveces funciona mejor y otras no, los MAPE indican esto como botan lo archivos de excel
